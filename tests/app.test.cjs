...
(async()=>{
 ...
 /* ---------- 분할된 앱 셸이 먼저 뜨고, 문항은 뒤따라 온다 ---------- */
 {
  ...
  check("문항 없이도 트랙 지도가 그려진다", early.nodes>0 && early.tracks===38, early);
 ...
 /* ---------- 콘텐츠 무결성 ---------- */
 {
  ...
  /* 전수 점검 — 이 파일이 보지 않는 아래를 본다(깨진 태그·빈 레슨·문항 중복).
     tools/content/audit_all.cjs 가 0건이 되도록 맞춰 두었으니 그대로 지킨다.
     경고를 남발하지 않도록 만든 도구라, 여기서 걸리면 대개 진짜다. */
  {
    let auditOut="", auditCode=0;
    try{
      auditOut=require("child_process").execFileSync(process.execPath,
        [path.join(__dirname,"..","tools","content","audit_all.cjs")],
        {encoding:"utf8", stdio:["ignore","pipe","pipe"]});
    }catch(e){ auditCode=1; auditOut=String((e.stdout||"")+(e.stderr||"")); }
    const head=auditOut.split("\n").filter(x=>x.indexOf("■")===0).slice(0,4);
    check("전수 점검에 고칠 것이 없다", auditCode===0, head);
  }
  ...
  /* 눈금이 26.1/24.6 에서 37.6/39.5 로 <b>올라간다.</b> 콘텐츠가 나빠져서가 아니라
     ...
     61차에 정답이 10자 이상 길던 328문항의 오답을 내용 있게 다시 썼다. 사람 눈금은
     31.5 → 27.3 (찍기 기준선 25%). 기계 눈금은 50.6 — 오답을 정답 길이의 5자 안으로
     맞추는 것을 원칙으로 했기 때문에, '정확히 가장 긴 것' 은 여전히 정답인 문항이
     많다. 사람이 알아채지 못하는 차이까지 없애려고 오답을 정답보다 길게 만들면
     '가장 긴 보기는 오답' 이라는 반대 단서가 생긴다. 기계 눈금은 이대로 두고
     사람 눈금을 지킨다.

     71차에 남아 있던 장식 하나를 더 찾았다 — 오답 뒤에 앞 문장을 되풀이하거나 8자
     토막을 덧붙여 길이를 맞춘 277개(`unrestate.cjs`). 걷어내자 사람 눈금이 29.0 까지
     올라왔고, 정답이 5자 이상 단독 최장이던 문항 520개의 오답을 전부 내용 있게 다시
     써서(`fix_opts.cjs` + `opt2_*.cjs`) 24.3 이 됐다 — 찍기 기준선(25%) 아래다.
     이제 5자 이상 차이 나는 문항은 0개다. 기계 눈금은 49.4. */
  const BIAS_EXACT=49.5, BIAS_HUMAN=24.5;
