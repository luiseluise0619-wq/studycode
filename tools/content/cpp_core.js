  ...
  q: [
    {
      ...
      test: { "test.cpp": "/* 클래스는 sol.cpp 안에 통째로 있다. 테스트에서 다시 선언하면 정의가 둘이 되어\n   링크가 되지 않으므로, 그대로 포함해 한 덩어리로 컴파일한다.\n   sol.cpp 는 인라인 멤버만 담고 있어 따로 컴파일해도 심볼이 생기지 않는다. */\n#include \"catch.hpp\"\n#include \"sol.cpp\"\n\nstatic int area_of(const Rect& r) { return r.area(); }\n\nTEST_CASE(\"넓이를 구한다\") {\n    Rect r(3, 4);\n    REQUIRE(r.area() == 12);\n    REQUIRE(r.width() == 3);\n}\n\nTEST_CASE(\"const 참조로 받아도 읽을 수 있다\") {\n    Rect r(5, 6);\n    REQUIRE(area_of(r) == 30);\n}\n\nTEST_CASE(\"const 객체에서도 부를 수 있다\") {\n    const Rect r(2, 7);\n    REQUIRE(r.area() == 14);\n    REQUIRE(r.width() == 2);\n}\n" },
      ex: "const 가 없는 멤버 함수는 const 객체에서 부를 수 없습니다. 읽기만 하는데도 컴파일이 막혀요. 값을 고치지 않는 함수에는 const 를 붙이는 것이 기본입니다.",
    },
    {
      k: "Fraction · 만들 때 약분해 두기",
      qq: "<code>Fraction</code> 을 완성하세요. <b>만드는 순간 약분</b>되어 <code>2/4</code> 와 <code>1/2</code> 가 <b>같은 것</b>으로 취급돼야 합니다.",
      src: "#include <numeric>\n\nclass Fraction {\n    int n_, d_;\npublic:\n    Fraction(int n, int d) : n_(n), d_(d) {}\n    int num() const { return n_; }\n    int den() const { return d_; }\n    bool operator==(const Fraction& o) const { return n_ == o.n_ && d_ == o.d_; }\n};\n",
      sol: "#include <numeric>\n\nclass Fraction {\n    int n_, d_;\npublic:\n    Fraction(int n, int d) {\n        int g = std::gcd(n < 0 ? -n : n, d < 0 ? -d : d);\n        if (g == 0) g = 1;\n        if (d < 0) { n = -n; d = -d; }\n        n_ = n / g;\n        d_ = d / g;\n    }\n    int num() const { return n_; }\n    int den() const { return d_; }\n    bool operator==(const Fraction& o) const { return n_ == o.n_ && d_ == o.d_; }\n};\n",
      test: { "test.cpp": "/* 클래스가 sol.cpp 안에 있으므로 그대로 포함해 한 덩어리로 컴파일한다 */\n#include \"catch.hpp\"\n#include \"sol.cpp\"\n\nTEST_CASE(\"만들 때 약분된다\") {\n    Fraction f(2, 4);\n    REQUIRE(f.num() == 1);\n    REQUIRE(f.den() == 2);\n}\n\nTEST_CASE(\"값이 같으면 같은 것\") {\n    REQUIRE(Fraction(2, 4) == Fraction(1, 2));\n    REQUIRE(Fraction(6, 3) == Fraction(2, 1));\n}\n\nTEST_CASE(\"다르면 다르다\") {\n    REQUIRE_FALSE(Fraction(1, 3) == Fraction(1, 2));\n}\n\nTEST_CASE(\"이미 기약이면 그대로\") {\n    Fraction f(3, 7);\n    REQUIRE(f.num() == 3);\n    REQUIRE(f.den() == 7);\n}\n\nTEST_CASE(\"분자가 0 이면 0/1\") {\n    Fraction f(0, 5);\n    REQUIRE(f.num() == 0);\n    REQUIRE(f.den() == 1);\n}\n\nTEST_CASE(\"부호는 분자로 모은다\") {\n    Fraction f(1, -2);\n    REQUIRE(f.num() == -1);\n    REQUIRE(f.den() == 2);\n}\n" },
      ex: "생성자가 받은 값을 그대로 담기만 하면 2/4 와 1/2 가 다른 객체가 됩니다. operator== 를 아무리 잘 써도 소용없어요. 이런 '항상 지켜져야 하는 성질' 은 생성자에서 한 번 맞춰 두는 것이 정석입니다 — 그래야 나중에 비교·해시·출력이 전부 자연스럽게 맞습니다.",
    },
  ],
},
{
