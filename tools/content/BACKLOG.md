...
40자 미만 해설 748개가 있었다. 거의 전부 정답을 되풀이한 것이었다 —
`"int 는 정수형이다"`, `"6 * 2 = 12 다"`, `"<iostream> 이 필요하다"`(이건 태그로 먹혀 화면에서
"이 필요하다" 만 보였다). 틀린 사람은 왜 틀렸는지 모른 채 다음 문항으로 간다.

**전부 다시 썼다.** 하나마다 '왜 맞는가' 와 '나머지 보기는 왜 아닌가' 를 함께 적어 80자 이상으로.
도구는 `ex_short.cjs` — 트랙별로 뽑아 보고(`node ex_short.cjs python`), 새 해설을 `ex_<track>.cjs`
스펙(`{track, fixes:[{k, ex0, ex}]}`)으로 적어 `--apply` 로 넣는다. 같은 제목(`k`)이 여러 개인
트랙이 많아서 지금 해설의 한 토막(`ex0`)으로 짝을 가린다. 전부 찾아야 쓴다(하나라도 못 찾으면
아무것도 안 바꾼다).

지킬 것
- 해설에 `<int>` `<Integer>` `<iostream>` 같은 꺾쇠 이름을 그대로 쓰면 브라우저가 태그로 먹는다.
  `&lt;` `&gt;` 로 적는다. 데이터에 섞여 있던 19곳도 같이 고쳤다.
- `plain()` 은 엔티티를 풀지 않으므로 `ex0` 에 `>` 를 쓰면 데이터의 `&gt;` 와 안 맞는다.
- 다시 늘지 않도록 `audit_all.cjs` 가 40자 미만을 고칠 것으로 잡는다(CI 에 물려 있다).
- 새 문항을 쓸 때 해설은 처음부터 '정답의 근거 + 오답이 그럴듯해 보이는 이유' 로 적는다.

## 보기 길이 편향 (59차에 드러남, 60차에 전부 드러남, 61차에 큰 것부터, 71차에 전부 갚음)

**71차:** 60차가 놓친 장식이 하나 더 있었다 — 오답 뒤에 **앞 문장을 되풀이**하거나 8자
토막을 덧붙여 길이를 맞춘 것("…크기를 그대로 돌려준다. 선언 크기를 쓴다"). 문장이 완전해서
문법으로는 안 걸렸다. `unrestate.cjs` 로 277개를 첫 문장만 남기자 사람 눈금이 27.3 → 29.0
으로 올라왔고(참 숫자), 정답이 5자 이상 단독 최장이던 **520문항**의 오답을 전부 내용 있게
다시 썼다(`fix_opts.cjs --apply opt2_*.cjs`, 배열 스펙 지원). 결과 **24.3%** — 찍기
기준선(25%) 아래. 5자 이상 차이 나는 문항 0개. 기계 눈금 49.4. 눈금은 24.5/49.5 로 박았고
`audit_all.cjs` 가 되풀이 덧붙임을 '고칠 것' 으로 잡는다. 이 항목은 닫혔다.
...
남은 5~9자 차이 문항 300여 개는 71차에 갚았다.
...
PLAYWRIGHT_CHROMIUM=... node tests/app.test.cjs                               # 174
```
...
**72차 — 무성의한 오답의 끝맺음.** 앱 테스트는 80회 이상 나오는 끝맺음만 봤는데, 그 문턱
아래에 `'…에러가 발생한다'(37개 중 정답 2.7%)`, `'…필요 없다'(51개, 3.9%)`, `'…문제가 없다'`,
`'…오류가 난다'`, `'…않아도 된다'` 같은 열네 가지가 숨어 있었다 — 오답이 "컴파일 오류가
난다", "구분할 방법이 없다" 처럼 한 줄짜리 무성의한 단정문이라 생기는 꼴이다. 517문항의
오답 570개를 **구체적인 틀린 답**으로 다시 썼다(`fix_opt1.cjs --apply opt3_*.cjs`):
"컴파일 오류가 난다" → "i 를 선언하기 전에 써서 컴파일러가 거부한다", "방법이 없다" →
"세션 하이재킹 - TLS 를 켜는 것 말고는 손쓸 길이 없다". 끝맺음으로 찍기 26.3 → 26.2,
15% 아래 끝맺음 0개. 앱 테스트가 이제 30회 이상·15% 아래 끝맺음을 잡는다.

새 오답을 쓸 때 지킬 것: **"오류가 난다"·"필요 없다"·"문제가 없다" 로 끝내지 않는다.**
어떤 오류가 왜 나는지, 무엇이 필요 없다고 착각하는지까지 적어야 틀린 답이 된다.
`fix_opt1.cjs` 가 그 끝맺음(SKEW 목록)을 거부한다.
