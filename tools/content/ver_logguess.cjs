/* 데이터에 들어 있는 로그 문항 전체를 놓고 <b>찍어서 맞힐 길</b>을 잰다.
   ...
   그보다 15%p 넘게 높은 규칙이 남아 있으면 갚아야 할 빚이다.

   기준선보다 <b>낮은</b> 쪽은 실패로 보지 않는다. 넘치면 '읽지 않아도 맞힌다' 이지만,
   모자라면 '그 줄 하나를 후보에서 뺀다' 일 뿐이라 힘이 훨씬 약하다 — 원인이 가장
   심각한 줄이거나 가장 짧은 줄인 경우가 드문 것은 장르의 성질이다. */
const fs = require("fs");
