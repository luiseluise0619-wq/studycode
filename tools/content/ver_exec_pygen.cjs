...
const Q=require(process.argv[2]||"./exec_py2.cjs");
const TMP="/tmp/claude-0/-home-user-studycode/48297e2f-2aa5-53f1-a08c-af741856ba9b/scratchpad/_pyx.py";
...
Q.forEach((q,i)=>{
  ...
  /* 7차에 이모지를 걷어내면서 이 정규식이 빈 채로 남아
     파일이 통째로 깨져 있었다. 이모지는 더 없으므로 길이만 본다. */
  if(String(q.ex).length<200) fail("해설이 200자 미만");
  if(q.tests.length<4) fail("tests 가 4개 미만");
  if(!q.edge||q.edge.length<1) fail("edge 가 없다");
  if(!new RegExp("(?:def|class)\\s+"+q.fn).test(q.src)) fail("src 에 "+q.fn+" 정의가 없다");
  if(!new RegExp("(?:def|class)\\s+"+q.fn).test(q.sol)) fail("sol 에 "+q.fn+" 정의가 없다");
