...
/* 파일을 여러 개 주면 한 트랙으로 합쳐서 잰다 — 길이 쏠림은 트랙 단위로 봐야
   의미가 있다. 배치 파일 하나는 표본이 84문항뿐이라 숫자가 심하게 흔들린다.
     node tools/content/ver_track_plain.cjs ./tools/content/track_test1.cjs ./tools/content/track_git2.cjs */
const FILES=process.argv.slice(2).filter(x=>x[0]!=="-");
const UNITS=(FILES.length?FILES:["./track_test1.cjs"])
...
/* 길이는 app.test.cjs 와 **똑같은 방식**으로 잰다 — 태그만 벗기고 엔티티는
   그대로 센다. 화면에 보이는 길이로 치면 &lt; 같은 것이 한 글자지만, 두 도구가
   서로 다른 숫자를 내면 여기서 통과한 것이 저기서 걸린다. 기준은 하나여야 한다. */
const width=o=>String(o).replace(/<[^>]*>/g,"").trim().length;
