...
/* 시작 코드는 대개 아무것도 안 하므로, 테스트가 만든 프라미스가 아무에게도
   잡히지 않은 채 남는다. 노드는 그것만으로 프로세스를 죽이지만 브라우저는
   그러지 않는다 — 앱과 같은 조건으로 맞춘다. */
...
/* 앱 채점기와 같이, 테스트 식이 프라미스를 돌려주면 기다렸다 비교한다.
   영영 안 끝나는 프라미스는 3초에서 끊는 것도 앱과 같다. */
function wait(v) {
  if (!v || typeof v.then !== 'function') return Promise.resolve(v);
  let id;
  const to = new Promise((_, rj) => { id = setTimeout(() => rj(new Error('시간 초과')), 3000); });
  return Promise.race([v, to]).finally(() => clearTimeout(id));
}

async function grade(code, q) {
  ...
  for (const t of (q.tests || [])) {
    let got; try { got = await wait(run(t.in)); } catch (e) { got = '[에러] ' + e.message; }
    let want; try { want = await wait(exp(t.out)); } catch (e) { want = Symbol('bad-out'); }
    if (eq(got, want)) out.pass++; else out.err = out.err || (t.in + ' → ' + JSON.stringify(got) + ' (기대 ' + t.out + ')');
  }
  for (const t of (q.edge || [])) {
    let got; try { got = await wait(run(t.in)); } catch (e) { got = '[에러] ' + e.message; }
    let want; try { want = await wait(exp(t.out)); } catch (e) { want = Symbol('bad-out'); }
...
for (let i = 0; i < Q.length; i++) {
  ...
  const sol = SOL && SOL[i];
  if (!sol) { say(at + ' 모범 답안이 없다'); continue; }
  const g = await grade(sol, q);
